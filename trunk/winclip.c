...
/**
 *当运行window exe文件时，如果参数使用文件路径在拖放时路径会转为posix风格。
 *需要设计一下：若拖放时按住Shift键忽略一切，强制转为Windows路径输入到控制台。
 */
static void
paste_hdrop(HDROP drop)
{
  //Added By Fang3s : 
  //IsCtrlPressed VK_CONTROL
  //IsAltPressed VK_MENU
  //IsShiftPressed VK_SHIFT
  ...
  bool using_win_path = is_key_down(VK_CONTROL);
  ...
  uint n = DragQueryFileW(drop, -1, 0, 0);
    ...
    if(using_win_path){
      ...
      fn = newn(char, MAX_PATH);
      strlcpy(fn, wfn, MAX_PATH);  
    }
    else{    
      uint wfn_len = DragQueryFileW(drop, i, 0, 0);
      wchar wfn[wfn_len + 1];
      DragQueryFileW(drop, i, wfn, wfn_len + 1);
      fn = cygwin_create_path(CCP_WIN_W_TO_POSIX, wfn);
    }
#else
    uint wfn_len = DragQueryFileA(drop, i, 0, 0);
    char wfn[wfn_len + 1];
...
#endif
	//对于windows路径处理：空格添加引号，其他不应该在这里处理，待修改。
	//当路径作为参数传递时，win路径间隔符\会被当做转义而丢失，三种解决办法：添加引号，用双个\\代替单个\，用/替代\。
